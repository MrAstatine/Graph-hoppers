...
def generate_nx_graph(topology):
    ...
    for i, j in G.edges():
        ...
        G.get_edge_data(i, j)[
            "numsp"
        ] = 0  # Indicates the number of shortest paths going through the link
...
class Env1(gym.Env):
    ...
    def __init__(self):
        ...
        # Set default action and observation spaces to avoid NoneType errors
    ...
    def set_spaces(self):
        # Set action_space and observation_space after environment is generated
        if self.ordered_edges is not None:
            self.action_space = spaces.Discrete(len(self.ordered_edges))
        if self.graph_state is not None:
            self.observation_space = spaces.Box(
                low=0, high=np.inf, shape=self.graph_state.shape, dtype=np.float32
            )
        ...
        for n1 in self.graph:
            for n2 in self.graph:
                if n1 != n2:
                    ...
                    [
                        self.allPaths[str(n1) + ":" + str(n2)].append(p)
                        for p in nx.all_simple_paths(
                            self.graph, source=n1, target=n2, cutoff=self.diameter * 2
                        )
                    ]

                    # We take all the paths from n1 to n2 and we order them according to the path length
                    self.allPaths[str(n1) + ":" + str(n2)] = sorted(
                        self.allPaths[str(n1) + ":" + str(n2)],
                        key=lambda item: (len(item), item),
                    )

                    path = 0
                    while path < self.K and path < len(
                        self.allPaths[str(n1) + ":" + str(n2)]
                    ):
                        ...
                        while j < len(currentPath):
                            self.graph.get_edge_data(currentPath[i], currentPath[j])[
                                "numsp"
                            ] = (
                    ...
                    del self.allPaths[str(n1) + ":" + str(n2)][
                        path : len(self.allPaths[str(n1) + ":" + str(n2)])
                    ]
                    gc.collect()
    ...
    def generate_environment(self, topology, listofdemands):
        ...
        # Set action and observation spaces now that environment is initialized
        self.set_spaces()
    ...
    def make_step(self, state, action, demand, source, destination):
        ...
        while j < len(currentPath):
            self.graph_state[
                self.edgesDict[str(currentPath[i]) + ":" + str(currentPath[j])]
            ][0] -= demand
            if (
                self.graph_state[
                    self.edgesDict[str(currentPath[i]) + ":" + str(currentPath[j])]
    ...
    def reset(self, *, seed=None, options=None):
        ...
        Args:
            ...
            options (dict, optional): Not used, for compatibility.
