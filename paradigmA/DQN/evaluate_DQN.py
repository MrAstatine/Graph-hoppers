...
class LBAgent:
    ...
    def act(self, env, state, demand, n1, n2):
        ...
        id_last_free = (
            -1
        )  # Indicates the id of the last free path where we will allocate the demand
        path = 0
        ...
        # If there is just one path to allocate we allocate it there
            ...
            while allocated == 0:
                ...
                can_allocate = (
                    1  # Indicates 1 if we can allocate the demand, 0 otherwise
                )
                i = 0
                j = 1
...
class DQNAgent:
    def __init__(self, env_nsfnet):
        ...
        self.bw_demand_feature = np.zeros(
            (env_nsfnet.numEdges, len(env_nsfnet.listofDemands))
        )
    ...
    def get_graph_features(self, env, copyGraph):
        ...
        sample = {
            ...
            "bw_allocated": tf.convert_to_tensor(
                value=self.bw_demand_feature, dtype=tf.float32
            ),
...
def exec_lb_model_episodes(experience_memory, graph_topology):
    env_lb = gym.make(ENV_NAME).unwrapped
    env_lb.seed(SEED)
    ...
    while iter_episode < len(experience_memory):
        if new_episode:
            ...
            new_state, reward, done, _, _, _ = env_lb.make_step(
            ...
            if experience_memory[iter_episode][0] != new_episode_it:
                print(
                    "LB ERROR! The experience replay buffer needs more samples/episode"
                )
                os.kill(os.getpid(), 9)
...
def exec_sap_model_episodes(experience_memory, graph_topology):
    env_sap = gym.make(ENV_NAME).unwrapped
    env_sap.seed(SEED)
    ...
    while iter_episode < len(experience_memory):
        if new_episode:
            ...
            new_state, reward, done, _, _, _ = env_sap.make_step(
                state, action, demand, source, destination
            )
            ...
            if experience_memory[iter_episode][0] != new_episode_it:
                print(
                    "SAP ERROR! The experience replay buffer needs more samples/episode"
                )
                os.kill(os.getpid(), 9)
    ...
    while iter_episode < len(experience_memory):
        if new_episode:
            ...
            action, state_action = agent.act(
                env_dqn, state, demand, source, destination, True
            )
            new_state, reward, done, new_demand, new_source, new_destination = (
                env_dqn.make_step(state, action, demand, source, destination)
            )
            rewardAdd = rewardAdd + reward
            state = new_state
            if done:
            ...
            if experience_memory[iter_episode][0] != new_episode_it:
                print(
                    "DQNAgent ERROR! The experience replay buffer needs more samples/episode"
                )
                os.kill(os.getpid(), 9)
...
if __name__ == "__main__":
    ...
    env_dqn = gym.make(ENV_NAME_AGENT).unwrapped
    env_dqn.seed(SEED)
    ...
    checkpoint = tf.train.Checkpoint(
        model=dqn_agent.primary_network, optimizer=dqn_agent.optimizer
    )
