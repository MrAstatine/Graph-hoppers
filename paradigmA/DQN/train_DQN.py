...
# below line is to disable GPU, comment it out if you want to use GPU. Remember to also comment out the line in run_training.py
...
"""
ITERATIONS = 500  # just to verify everything works
TRAINING_EPISODES = 5
...
class DQNAgent:
    def __init__(self, batch_size):
        ...
        self.writer = tf.summary.create_file_writer(train_dir)
        self.K = 4  # K-paths
        self.listQValues = None
        self.numbersamples = batch_size
        self.action = None
        self.capacity_feature = None
        self.bw_allocated_feature = np.zeros(
            (env_training.numEdges, len(env_training.listofDemands))
        )
        ...
        self.optimizer = tf.keras.optimizers.SGD(
            learning_rate=hparams["learning_rate"], momentum=0.9, nesterov=True
        )
    ...
    def act(self, env, state, demand, source, destination, flagEvaluation):
        ...
        while path < len(pathList):
            ...
            while j < len(currentPath):
                state_copy[
                    env.edgesDict[str(currentPath[i]) + ":" + str(currentPath[j])]
                ][1] = demand
                i = i + 1
                j = j + 1
        ...
        graph_ids = [
            tf.fill([tf.shape(vs[it]["link_state"])[0]], it)
            for it in range(len(list_k_features))
        ]
        ...
        tensors = {
            ...
            "first": tf.concat(
                [v["first"] + m for v, m in zip(vs, first_offset)], axis=0
            ),
            "second": tf.concat(
                [v["second"] + m for v, m in zip(vs, second_offset)], axis=0
            ),
    ...
    def get_graph_features(self, env, copyGraph):
        ...
        sample = {
            ...
            "betweenness": tf.convert_to_tensor(
                value=env.between_feature, dtype=tf.float32
            ),
            "bw_allocated": tf.convert_to_tensor(
                value=self.bw_allocated_feature, dtype=tf.float32
            ),
            "capacities": tf.convert_to_tensor(
                value=self.capacity_feature, dtype=tf.float32
            ),
            "first": tf.convert_to_tensor(env.first, dtype=tf.int32),
            "second": tf.convert_to_tensor(env.second, dtype=tf.int32),
        }

        sample["capacities"] = tf.reshape(
            sample["capacities"][0 : sample["num_edges"]], [sample["num_edges"], 1]
        )
        sample["betweenness"] = tf.reshape(
            sample["betweenness"][0 : sample["num_edges"]], [sample["num_edges"], 1]
        )

        hiddenStates = tf.concat(
            [sample["capacities"], sample["betweenness"], sample["bw_allocated"]],
            axis=1,
        )

        paddings = tf.constant(
            [[0, 0], [0, hparams["link_state_dim"] - 2 - hparams["num_demands"]]]
        )
        ...
        if self.writer is not None:
            with self.writer.as_default():
                ...
                tf.summary.histogram(
                    name="gradients_5", data=gradients[5], step=self.global_step
                )
                tf.summary.histogram(
                    name="gradients_7", data=gradients[7], step=self.global_step
                )
                tf.summary.histogram(
                    name="gradients_9", data=gradients[9], step=self.global_step
                )
            tf.summary.histogram(
                name="FirstLayer/kernel:0",
                data=self.primary_network.variables[0],
                step=self.global_step,
            )
            tf.summary.histogram(
                name="FirstLayer/bias:0",
                data=self.primary_network.variables[1],
                step=self.global_step,
            )
            tf.summary.histogram(
                name="kernel:0",
                data=self.primary_network.variables[2],
                step=self.global_step,
            )
            tf.summary.histogram(
                name="recurrent_kernel:0",
                data=self.primary_network.variables[3],
                step=self.global_step,
            )
            tf.summary.histogram(
                name="bias:0",
                data=self.primary_network.variables[4],
                step=self.global_step,
            )
            tf.summary.histogram(
                name="Readout1/kernel:0",
                data=self.primary_network.variables[5],
                step=self.global_step,
            )
            tf.summary.histogram(
                name="Readout1/bias:0",
                data=self.primary_network.variables[6],
                step=self.global_step,
            )
            tf.summary.histogram(
                name="Readout2/kernel:0",
                data=self.primary_network.variables[7],
                step=self.global_step,
            )
            tf.summary.histogram(
                name="Readout2/bias:0",
                data=self.primary_network.variables[8],
                step=self.global_step,
            )
            tf.summary.histogram(
                name="Readout3/kernel:0",
                data=self.primary_network.variables[9],
                step=self.global_step,
            )
            tf.summary.histogram(
                name="Readout3/bias:0",
                data=self.primary_network.variables[10],
                step=self.global_step,
            )
            self.writer.flush()
    ...
    def _forward_pass(self, x):
        prediction_state = self.primary_network(
            x[0], x[1], x[2], x[3], x[4], training=True
        )
        preds_next_target = tf.stop_gradient(
            self.target_network(x[6], x[7], x[9], x[10], x[11], training=True)
        )
    ...
    def _train_step(self, batch):
        ...
        with tf.GradientTape() as tape:
                ...
                target.append(
                    tf.stop_gradient(
                        [
                            x[5]
                            + self.gamma
                            * tf.math.reduce_max(preds_next_target)
                            * (1 - x[8])
                        ]
                    )
                )
            ...
            loss = tf.keras.losses.MSE(
                tf.stack(target, axis=1), tf.stack(preds_state, axis=1)
            )
        ...
        # Skip None gradients (disconnected vars) before clipping/applying.
        gradients_and_vars = [
            (tf.clip_by_value(gradient, -1.0, 1.0), variable)
            for gradient, variable in zip(grad, self.primary_network.variables)
    ...
    def replay(self, episode):
        for i in range(MULTI_FACTOR_BATCH):
            ...
            if i % store_loss == 0:
                loss_value = float(tf.reduce_mean(loss).numpy())
                fileLogs.write(".," + "%.9f" % loss_value + ",\n")
    ...
    def add_sample(
        ...
        self.memory.append(
            (
                state_action["link_state"],
                state_action["graph_id"],
                state_action["first"],  # 2
                state_action["second"],
                tf.convert_to_tensor(state_action["num_edges"]),  # 4
                tf.convert_to_tensor(reward, dtype=tf.float32),
                tensors["link_state"],
                tensors["graph_id"],  # 7
                tf.convert_to_tensor(int(done), dtype=tf.float32),
                tensors["first"],
                tensors["second"],  # 10
                tf.convert_to_tensor(tensors["num_edges"]),
            )
        )  # 12
...
if __name__ == "__main__":
    ...
    checkpoint = tf.train.Checkpoint(
        model=agent.primary_network, optimizer=agent.optimizer
    )
    ...
    for eps in range(EVALUATION_EPISODES):
        ...
        while 1:
            ...
            new_state, reward, done, demand, source, destination = env_eval.make_step(
        ...
        for _ in range(train_episodes):
            ...
            while 1:
                ...
                action, state_action = agent.act(
                    env_training, state, demand, source, destination, False
                )
                new_state, reward, done, new_demand, new_source, new_destination = (
                    env_training.make_step(state, action, demand, source, destination)
                )
        ...
        if ep_it % evaluation_interval == 0:
            for eps in range(EVALUATION_EPISODES):
                ...
                while 1:
                    # We execute evaluation over current state
                    action, _ = agent.act(
                        env_eval, state, demand, source, destination, True
                    )
            ...
            fileLogs.write(
                "MAX REWD: " + str(max_reward) + " MODEL_ID: " + str(reward_id) + ",\n"
            )
